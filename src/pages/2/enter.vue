<template>
	<div class="s-register">
		...
		<header class="s-header">
			<div>
				<a href="#/"><img src="/static/icon/mingtang.png" alt="" class="logo"/></a>
				<span>会员登录</span>
			</div>
		</header>
		<!--注册框-->
		<div class="s-zhuce">
			...
			<div class="s-zhuce-wrapper">
				<div class="s-choose-style">
					<p>登录</p>
					<!--<router-link to="/phoneEnter" tag="p">手机验证登录<img src="../../images/icon/s-phone.png" alt="" /></router-link>-->
				</div>
				<ul class="s-import">
					<li>
						<p><input type="text" placeholder="请输入手机号" @blur="foucs($event)" class="number1"/>
					...
					<li>
						<p><input type="password" placeholder="请输入密码(6-20位号码字符)" @blur="pass($event)" class="password1"/>
							<img src="../../images/icon/check .png" alt="" /></p>
						<p>
							<!--<img src="../../images/icon/warning.png" alt="" />
					...
					<li class="s-click">
						<span @click="login()">登录</span><router-link to="/register" tag="span">会员注册</router-link>
					</li>
					<!--<li class="s-hint">-->
...
<script>
	export default {
      ...
      login :function(){
					...
					this.$axios.get('/api/enter?name='+name+'&pass='+pass).then(function(data){
